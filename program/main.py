...
def procedurehuffman(f):
    ...
    while len(h) > 1:
        ...
        while fl in visited_fs:  # Проверка на уникальность значения fs
            letter = chr(ord_val)  # Преобразовать численное значение в символ
            # Добавить букву к значению fs через пробел
            fl = str(fs) + " " + letter
            ord_val += 1  # Увеличить численное значение для следующей буквы
        visited_fs.add(fl)  # Добавление fs во множество посещенных значений
